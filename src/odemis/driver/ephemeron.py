...
from odemis.model import Detector, HwError, oneway, MD_ACQ_DATE
...
class MightyEbic(Detector):
    ...
    def _acquire_thread(self) -> None:
        ...
        try:
            ...
            asyncio.run(self._opc_client.start_scan(self.dwellTime.value, 0, 1, 0, res[0], res[1]))

            st = asyncio.run(self._opc_client.get_scan_time(self.dwellTime.value, res[0], res[1]))
            end_time = time.time() + 60 + st
...
class UaClient:
    ...
    async def initialize_client(self):
        async with self.client:
            ...
            loop_thread = threading.Thread(target=self.start_event_loop, args=(self._loop,), daemon=True)
            loop_thread.start()
    ...
    async def get_scan_time(self, dt, res_fast, res_slow) -> float:
        async with self.client:
            # dt pf, ps
    ...
    async def start_scan(self, req_dt, oversampling, channels, delay, p_fast, p_slow, sim=False):
        """
