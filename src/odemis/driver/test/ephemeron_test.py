...
from odemis.driver.ephemeron import STATE_NAME_BUSY
...
class TestMightyEBICDetector(unittest.TestCase):
    ...
    def test_acquisition_stop(self):
        ...
        # do a start acquisition with start and end time
        start = time.time()
        acq_thread = threading.Thread(target=self.start_acquisition_thread, name="EBIC acquisition thread")
        acq_thread.start()
        ...
        while acq_thread.is_alive():
            time.sleep(interval)
            count += interval
            if self.ebic_det._opc_client.controller_state == ephemeron.STATE_NAME_BUSY and count == 3:
                # call the stop_scan method
                self.ebic_det.stop_acquire()
                break
    ...
    def test_request_very_high_resolution(self):
        ...
        # take the max (4096, 4096) fov of the sem detector as resolution to scan
        pass
    ...
    def start_acquisition_thread(self):
        try:
            ...
            logging.debug("EBIC acquisition done")
